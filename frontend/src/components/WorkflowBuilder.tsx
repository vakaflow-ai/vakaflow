...
function SortableStep({ step, isFirst, canDelete, onSetFirst, onStepClick, onStepUpdate, onStepDelete, approverGroups, canEdit }: SortableStepProps) {
    ...
    >
      ...
      {canEdit && canDelete && (
        <button
          ...
          className="flex-shrink-0 px-3 py-1.5 text-xs font-medium text-red-600 bg-red-100 border border-red-300 rounded-md hover:bg-red-200 transition-colors"
...
export default function WorkflowBuilder({ steps, onStepsChange, onSetFirstStep, onStepClick, onStepDelete, approverGroups, canEdit = false }: WorkflowBuilderProps) {
  ...
  const handleStepDelete = (stepNumber: number) => {
    const updatedSteps = steps.filter(s => s.step_number !== stepNumber)
          ...
          <div className="space-y-3">
            {steps.map((step) => (
              <SortableStep
                ...
                onStepDelete={canEdit ? (onStepDelete || handleStepDelete) : undefined}
