...
// Removed: RequirementsSelectionStep - Compliance is managed outside form designer
// This function has been removed as compliance requirements should be managed separately
function _Removed_RequirementsSelectionStep({
  requirements,
  ...
  setSelectedRequirements: (selected: Set<string>) => void
  requirementsFilter: string
  setRequirementsFilter: (filter: string) => void
  filterByRisk: boolean
  setFilterByRisk: (value: boolean) => void
  filterByComplianceFramework: string
  setFilterByComplianceFramework: (value: string) => void
  filterByFunctionalArea: string
  setFilterByFunctionalArea: (value: string) => void
  editingLayout: Partial<FormLayoutCreate> | null
}) {
  // Extract unique filter values from requirements
  const complianceFrameworks = useMemo(() => {
    const frameworks = new Set<string>()
    requirements.forEach((req) => {
      if (req.section === 'Compliance Frameworks' && req.source_name) {
        frameworks.add(req.source_name)
      }
    })
  ...
  const functionalAreas = useMemo(() => {
    ...
    requirements.forEach((req) => {
      if (req.section?.startsWith('Functional Areas - ')) {
        const area = req.section.replace('Functional Areas - ', '')
        areas.add(area)
      }
    })
  ...
  const filteredRequirements = useMemo(() => {
    ...
    // Filter by Risk
    if (filterByRisk) {
      filtered = filtered.filter((req) => req.section === 'Risks')
    }

    // Filter by Compliance Framework
    if (filterByComplianceFramework) {
      filtered = filtered.filter(
        (req) => req.section === 'Compliance Frameworks' && req.source_name === filterByComplianceFramework
      )
    }
    ...
    if (filterByFunctionalArea) {
      filtered = filtered.filter(
        (req) => req.section === `Functional Areas - ${filterByFunctionalArea}`
      )
    }

    // If no filters are active, show all requirements
    if (!filterByRisk && !filterByComplianceFramework && !filterByFunctionalArea) {
      filtered = requirements
    }

    // Filter by search term
    if (requirementsFilter.trim()) {
      const filterLower = requirementsFilter.toLowerCase()
      filtered = filtered.filter(
        (req) =>
          req.label?.toLowerCase().includes(filterLower) ||
          req.field_name?.toLowerCase().includes(filterLower) ||
          req.description?.toLowerCase().includes(filterLower) ||
          req.source_name?.toLowerCase().includes(filterLower)
      )
    }

    // Remove any requirements that don't have proper mapping (should not exist, but filter them out)
    filtered = filtered.filter(
      (req) =>
        req.section === 'Risks' ||
    ...
    return filtered
  }, [requirements, filterByRisk, filterByComplianceFramework, filterByFunctionalArea, requirementsFilter])

  const toggleRequirementSelection = (fieldName: string) => {
  ...
  const selectAllFiltered = () => {
    ...
    filteredRequirements.forEach((req) => {
      if (req.field_name) {
        newSelected.add(req.field_name)
      }
    })
    setSelectedRequirements(newSelected)
  }
  ...
  const deselectAllFiltered = () => {
    ...
    filteredRequirements.forEach((req) => {
      if (req.field_name) {
        newSelected.delete(req.field_name)
      }
    })
    setSelectedRequirements(newSelected)
  }
  ...
  const hasActiveFilters = filterByRisk || filterByComplianceFramework || filterByFunctionalArea || requirementsFilter.trim() !== ''

  const totalSelected = selectedRequirements.size
    ...
    <div className="space-y-4">
      <div className="mb-3 shadow-md-elevation-2 rounded-md border border-gray-100 bg-white p-6 bg-primary-50 border-primary-200">
        <div className="mb-2 pb-2 border-b border-primary-200">
          <h2>Step 3: Compliance & Requirements</h2>
          <p className="text-caption text-gray-700 mt-1">
            Select the GRC controls and requirements that need to be evaluated for this form. These will be added to the agent submission process.
          </p>
        </div>
      </div>
      ...
      <div className="shadow-md-elevation-2 rounded-md border border-gray-100 bg-white p-6">
        <div className="mb-4">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-label text-gray-700">Filter Requirements</h3>
            {hasActiveFilters && (
              <button
                onClick={clearAllFilters}
                className="text-caption text-primary hover:text-blue-600 font-medium"
              >
                Clear All Filters
              </button>
            )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
            {/* Risk Filter */}
            <div>
              <label className="enterprise-label">By Risk</label>
              <button
                onClick={() => {
                ...
                className={`w-full px-3 py-2 text-body rounded-md border transition-all flex items-center justify-center gap-2 ${
                  filterByRisk
            ...
            <div>
              <label className="enterprise-label">By Compliance Framework</label>
              <select
            ...
            <div>
              <label className="enterprise-label">By Functional Area</label>
              <select
            ...
            <div>
              ...
              <div className="relative">
                <Search className="absolute left-md-3 top-1/2 transform -translate-y-1/2 text-gray-600 w-4 h-4" />
                <input
                  type="text"
        ...
        <div className="flex items-center justify-between pt-3 border-t border-gray-200">
          <div className="text-body text-gray-600">
            Showing <span className="font-medium text-gray-900">{totalFiltered}</span> of <span className="font-medium">{totalRequirements}</span> requirements
            {hasActiveFilters && (
              <span className="ml-2 text-caption text-primary">(filtered)</span>
            )}
          </div>
          <div className="flex items-center gap-2">
            <div className="text-body text-gray-600">
              <span className="font-medium text-primary">{totalSelected}</span> selected
            </div>
            {totalFiltered > 0 && (
              <>
                <button
                  onClick={selectAllFiltered}
                  className="text-caption px-2 py-1 bg-primary-100 text-primary-700 rounded hover:bg-primary-200 transition-colors"
                >
                  Select All
                </button>
                <button
                  onClick={deselectAllFiltered}
                  className="text-caption px-2 py-1 bg-gray-100 text-gray-700 rounded hover:bg-gray-200 transition-colors"
                >
                  Deselect All
                </button>
              </>
      ...
      <div className="shadow-md-elevation-2 rounded-md border border-gray-100 bg-white p-6 overflow-hidden">
        {filteredRequirements.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-body text-gray-500 mb-2">No requirements match the current filters</p>
            {hasActiveFilters && (
              <button
                onClick={clearAllFilters}
                className="text-body text-primary hover:text-blue-600 font-medium"
              >
              ...
              <thead>
                <tr className="bg-gray-50 border-b-2 border-gray-300">
                  <th className="table-header px-3 py-3 text-left">
                    <input
                      type="checkbox"
                      checked={filteredRequirements.length > 0 && filteredRequirements.every(req => req.field_name && selectedRequirements.has(req.field_name))}
                      onChange={(e) => {
                        if (e.target.checked) {
                          selectAllFiltered()
                        } else {
                  ...
                  <th className="table-header px-3 py-3 text-left">Requirement</th>
                  <th className="table-header px-3 py-3 text-left">Classification</th>
                  <th className="table-header px-3 py-3 text-left">Description</th>
                  <th className="table-header px-3 py-3 text-left">Field Name</th>
                </tr>
              </thead>
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredRequirements.map((req, idx) => {
                  const isSelected = req.field_name ? selectedRequirements.has(req.field_name) : false
                  const section = req.section || ''
                  const isRisk = section === 'Risks'
                  const isCompliance = section === 'Compliance Frameworks'
                  const isFunctionalArea = section?.startsWith('Functional Areas - ')
                  const functionalAreaName = isFunctionalArea ? section.replace('Functional Areas - ', '') : ''

                  return (
                    <tr
                      key={`${req.id || req.field_name}-${idx}`}
                      className={`hover:bg-primary-50/50 transition-colors cursor-pointer ${
                        isSelected ? 'bg-primary-50' : ''
                      }`}
                      onClick={() => req.field_name && toggleRequirementSelection(req.field_name)}
                    >
                      <td className="px-3 py-3 whitespace-nowrap">
                        <input
                          type="checkbox"
                          checked={isSelected}
                          onChange={() => req.field_name && toggleRequirementSelection(req.field_name)}
                      ...
                      <td className="px-3 py-3">
                        <div className="flex items-center gap-2 flex-wrap">
                          <span className="table-cell-primary">{req.label}</span>
                          {req.is_required && (
                            <span className="badge-text bg-red-100 text-red-700 px-1.5 py-0.5 rounded">Required</span>
                          )}
                        </div>
                      </td>
                      <td className="px-3 py-3">
                        <div className="flex flex-wrap gap-1.5">
                          {isRisk && (
                            <span className="badge-text bg-orange-100 text-orange-700 px-1.5 py-0.5 rounded border border-orange-200 flex items-center gap-1">
                              <AlertTriangle className="w-3 h-3" />
                              Risk
                            </span>
                          )}
                          {isCompliance && req.source_name && (
                            <span className="badge-text bg-primary-100 text-primary-700 px-1.5 py-0.5 rounded border border-primary-200 flex items-center gap-1">
                              <Shield className="w-3 h-3" />
                              {req.source_name}
                            </span>
                          )}
                          {isFunctionalArea && (
                            <span className="badge-text bg-blue-100 text-blue-600 px-1.5 py-0.5 rounded border border-blue-400">
                              {functionalAreaName}
                            </span>
                          )}
                        </div>
                      ...
                      <td className="px-3 py-3">
                        <p className="table-cell-secondary line-clamp-2" title={req.description || ''}>
                          {req.description || <span className="text-gray-600 italic">No description</span>}
                        </p>
                      </td>
                      <td className="px-3 py-3">
                        <code 
                          className="table-cell-meta bg-gray-100 px-1.5 py-0.5 rounded font-mono block" 
                          title={req.field_name}
      ...
      <div className="rounded-md border border-gray-100 bg-white p-6 shadow-sm bg-blue-50 border-blue-400">
        <p className="text-body text-blue-900">
          <span className="font-medium">💡 Tip:</span> Use the filters above to find requirements by Risk type, Compliance Framework (GDPR, SOC 2, ISO 27001, etc.), or Functional Area (IT, Security, HR, Finance, Legal, Operations). 
          Selected requirements will be added to the form layout and will appear in the agent submission process for evaluation.
        </p>
      </div>
    </div>
  )
...
function ReviewAndSubmitStep({
  ...
  const totalFields = guidedSteps.reduce((sum, step) => sum + (step.field_names?.length || 0), 0)

  // Get step names
  const stepNames = guidedSteps
    .sort((a, b) => a.step_number - b.step_number)
    .map(step => step.title)
    .join(', ')
