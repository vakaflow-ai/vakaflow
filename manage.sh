...
case "${1:-}" in
    ...
    rebuild)
        echo -e "${BLUE}=== Rebuilding VAKA Platform ===${NC}"
        echo ""
        ...
        if [ -d "venv" ]; then
            ...
            echo -e "${YELLOW}Reinstalling backend dependencies...${NC}"
            pip install -r requirements.txt --force-reinstall --no-cache-dir || {
                echo -e "${RED}Failed to reinstall backend dependencies${NC}"
                exit 1
            }
            deactivate
        else
        ...
        echo -e "${GREEN}✓ Backend dependencies rebuilt${NC}"
        echo ""
        ...
        npm install || {
            echo -e "${RED}Failed to reinstall frontend dependencies${NC}"
            exit 1
        }
        ...
        start_backend
        if [ $? -ne 0 ]; then
            echo -e "${RED}Backend start failed after rebuild. Check logs: tail -f .backend.log${NC}"
            exit 1
        fi
        ...
        start_frontend
        if [ $? -ne 0 ]; then
            echo -e "${RED}Frontend start failed after rebuild. Check logs: tail -f .frontend.log${NC}"
            exit 1
        fi
    ...
    rebuild-code)
        echo -e "${BLUE}=== Reloading Latest Code ===${NC}"
        echo -e "${YELLOW}Note: This keeps dependencies and database intact, only reloads code${NC}"
        echo ""
        ...
        # Start services (will pick up latest code)
        start_backend
        if [ $? -ne 0 ]; then
            echo -e "${RED}Backend start failed. Check logs: tail -f .backend.log${NC}"
            exit 1
        fi
        ...
        start_frontend
        if [ $? -ne 0 ]; then
            echo -e "${RED}Frontend start failed. Check logs: tail -f .frontend.log${NC}"
            exit 1
        fi
        
        echo ""
        echo -e "${GREEN}=== Latest Code Reloaded ===${NC}"
        echo ""
        show_status
        ;;
    ...
    *)
        ...
        echo ""
        echo "Usage: $0 [start|stop|restart|status|logs|rebuild|rebuild-code]"
        echo ""
        echo "Commands:"
        ...
        echo "  rebuild      - Rebuild dependencies and restart services"
        echo "  rebuild-code - Reload latest code (keeps dependencies and database)"
        echo ""
        exit 1
        ;;
esac
